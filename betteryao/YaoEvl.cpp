...
static log4cxx::LoggerPtr logger(log4cxx::Logger::getLogger("YaoEvl.cpp"));
...
void Yao::circuit_evaluate()
{
	...
	G M;
        ...
        // evl will receive the masked gen input and initialize its circuits with the input
	...
	m_comm_sz += m_gen_inp_masks[0].size();
        ...
        // eval receives the constant keys for its circuit
        // (remember in HBC it only has one circuit, m_gcs[0])
        // so that it may evaluate the circuit
	...
	EVL_END
          ...
          // it seems to me that these should happen at the beginning
	...
	trim_output(m_gcs[0]);
        // this will be m_gcs[0].trim_output();
